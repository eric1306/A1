...
{
	UE_LOG(LogA1Teams, Error, TEXT("You can't set the team ID on a player bot controller (%s); it's driven by the associated player state"), *GetPathNameSafe(this));
}

FGenericTeamId ALyraPlayerBotController::GetGenericTeamId() const
{
	if (IA1TeamAgentInterface* PSWithTeamInterface = Cast<IA1TeamAgentInterface>(PlayerState))
	{
...
FOnA1TeamIndexChangedDelegate* ALyraPlayerBotController::GetOnTeamIndexChangedDelegate()
{
