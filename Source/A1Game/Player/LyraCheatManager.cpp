...
DEFINE_LOG_CATEGORY(LogA1Cheat);
...
void ULyraCheatManager::InitCheatManager()
{
	...
	if (GIsEditor)
	{
		...
		for (const FLyraCheatToRun& CheatRow : GetDefault<UA1DeveloperSettings>()->CheatsToRun)
		{
...
void ULyraCheatManager::CheatOutputText(const FString& TextToOutput)
{
	...
	// Output to log.
	UE_LOG(LogA1Cheat, Display, TEXT("%s"), *TextToOutput);
...
{
	FGameplayTag Tag = A1GameplayTags::FindTagByString(TagName, true);
	if (Tag.IsValid())
	{
		if (ULyraAbilitySystemComponent* LyraASC = GetPlayerAbilitySystemComponent())
		{
	...
	{
		UE_LOG(LogA1Cheat, Display, TEXT("AddTagToSelf: Could not find any tag matching [%s]."), *TagName);
	}
}
...
void ULyraCheatManager::RemoveTagFromSelf(FString TagName)
{
	...
	{
		UE_LOG(LogA1Cheat, Display, TEXT("RemoveTagFromSelf: Could not find any tag matching [%s]."), *TagName);
	}
}
...
void ULyraCheatManager::ApplySetByCallerDamage(ULyraAbilitySystemComponent* LyraASC, float DamageAmount)
{
	...
	{
		SpecHandle.Data->SetSetByCallerMagnitude(A1GameplayTags::SetByCaller_IncomingDamage, DamageAmount);
...
void ULyraCheatManager::ApplySetByCallerHeal(ULyraAbilitySystemComponent* LyraASC, float HealAmount)
{
	...
	if (SpecHandle.IsValid())
	{
		SpecHandle.Data->SetSetByCallerMagnitude(A1GameplayTags::SetByCaller_IncomingHealth, HealAmount);
		LyraASC->ApplyGameplayEffectSpecToSelf(*SpecHandle.Data.Get());
	}
}
...
void ULyraCheatManager::DamageSelfDestruct()
{
	if (ALyraPlayerController* LyraPC = Cast<ALyraPlayerController>(GetOuterAPlayerController()))
	{
		...
		{
			if (PawnExtComp->HasReachedInitState(A1GameplayTags::InitState_GameplayReady))
			{
...
void ULyraCheatManager::God()
{
	if (ALyraPlayerController* LyraPC = Cast<ALyraPlayerController>(GetOuterAPlayerController()))
	{
		...
		{
			const FGameplayTag Tag = A1GameplayTags::Cheat_GodMode;
...
void ULyraCheatManager::UnlimitedHealth(int32 Enabled)
{
	if (ULyraAbilitySystemComponent* LyraASC = GetPlayerAbilitySystemComponent())
	{
		const FGameplayTag Tag = A1GameplayTags::Cheat_UnlimitedHealth;
		const bool bHasTag = LyraASC->HasMatchingGameplayTag(Tag);
