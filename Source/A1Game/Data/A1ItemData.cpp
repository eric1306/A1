...
#include UE_INLINE_GENERATED_CPP_BY_NAME(A1ItemData)

const UA1ItemData& UA1ItemData::Get()
{
	return ULyraAssetManager::Get().GetItemData();
}

#if WITH_EDITORONLY_DATA
void UA1ItemData::PreSave(FObjectPreSaveContext SaveContext)
{
	...
	for (const auto& Pair : ItemTemplateIDToClass)
	{
		...
		const UA1ItemTemplate* ItemTemplate = Pair.Value.GetDefaultObject();
		if (const UA1ItemFragment_Equipable_Weapon* WeaponFragment = ItemTemplate->FindFragmentByClass<UA1ItemFragment_Equipable_Weapon>())
		{
		...
		else if (ItemTemplate->FindFragmentByClass<UA1ItemFragment_Equipable_Armor>())
		{
...
const UA1ItemTemplate& UA1ItemData::FindItemTemplateByID(int32 ItemTemplateID) const
{
	const TSubclassOf<UA1ItemTemplate>* ItemTemplateClass = ItemTemplateIDToClass.Find(ItemTemplateID);
...
int32 UA1ItemData::FindItemTemplateIDByClass(TSubclassOf<UA1ItemTemplate> ItemTemplateClass) const
{
...
void UA1ItemData::GetAllItemTemplateClasses(TArray<TSubclassOf<UA1ItemTemplate>>& OutItemTemplateClasses) const
{
