...
void ULyraHeroComponent::OnRegister()
{
	...
	if (!GetPawn<APawn>())
	{
		UE_LOG(LogA1, Error, TEXT("[ULyraHeroComponent::OnRegister] This component has been added to a blueprint whose base class is not a Pawn. To use this component, it MUST be placed on a Pawn Blueprint."));
...
bool ULyraHeroComponent::CanChangeInitState(UGameFrameworkComponentManager* Manager, FGameplayTag CurrentState, FGameplayTag DesiredState) const
{
	...
	if (!CurrentState.IsValid() && DesiredState == A1GameplayTags::InitState_Spawned)
	{
	...
	else if (CurrentState == A1GameplayTags::InitState_Spawned && DesiredState == A1GameplayTags::InitState_DataAvailable)
	{
	...
	else if (CurrentState == A1GameplayTags::InitState_DataAvailable && DesiredState == A1GameplayTags::InitState_DataInitialized)
	{
		// Wait for player state and extension component
		ALyraPlayerState* LyraPS = GetPlayerState<ALyraPlayerState>();

		return LyraPS && Manager->HasFeatureReachedInitState(Pawn, ULyraPawnExtensionComponent::NAME_ActorFeatureName, A1GameplayTags::InitState_DataInitialized);
	}
	else if (CurrentState == A1GameplayTags::InitState_DataInitialized && DesiredState == A1GameplayTags::InitState_GameplayReady)
	{
...
{
	if (CurrentState == A1GameplayTags::InitState_DataAvailable && DesiredState == A1GameplayTags::InitState_DataInitialized)
	{
		APawn* Pawn = GetPawn<APawn>();
		ALyraPlayerState* LyraPS = GetPlayerState<ALyraPlayerState>();
...
void ULyraHeroComponent::OnActorInitStateChanged(const FActorInitStateChangedParams& Params)
{
	...
	{
		if (Params.FeatureState == A1GameplayTags::InitState_DataInitialized)
		{
...
{
	static const TArray<FGameplayTag> StateChain = { A1GameplayTags::InitState_Spawned, A1GameplayTags::InitState_DataAvailable, A1GameplayTags::InitState_DataInitialized, A1GameplayTags::InitState_GameplayReady };
...
void ULyraHeroComponent::BeginPlay()
{
	...
	ensure(TryToChangeInitState(A1GameplayTags::InitState_Spawned));
	CheckDefaultInitialization();
}
		...
		if (const ULyraPawnData* PawnData = PawnExtComp->GetPawnData<ULyraPawnData>())
		{
			if (const ULyraInputConfig* InputConfig = PawnData->InputConfig)
			{
				...
				if (ensureMsgf(LyraIC, TEXT("Unexpected Input Component class! The Gameplay Abilities will not be bound to their inputs. Change the input component to ULyraInputComponent or a subclass of it.")))
				{
					...
					LyraIC->BindNativeAction(InputConfig, A1GameplayTags::InputTag_Jump, ETriggerEvent::Triggered, this, &ThisClass::Input_Jump, /*bLogIfNotFound=*/ false);
...
void ULyraHeroComponent::Input_Jump(const FInputActionValue& InputActionValue)
{
	ACharacter* Pawn = GetPawn<ACharacter>();
	Pawn->Jump();
}
