...
void ULyraPawnExtensionComponent::SetPawnData(const ULyraPawnData* InPawnData)
{
	...
	if (PawnData)
	{
		UE_LOG(LogA1, Error, TEXT("Trying to set PawnData [%s] on pawn [%s] that already has valid PawnData [%s]."), *GetNameSafe(InPawnData), *GetNameSafe(Pawn), *GetNameSafe(PawnData));
		return;
	}
...
void ULyraPawnExtensionComponent::InitializeAbilitySystem(ULyraAbilitySystemComponent* InASC, AActor* InOwnerActor)
{
	...
	UE_LOG(LogA1, Verbose, TEXT("Setting up ASC [%s] on pawn [%s] owner [%s], existing [%s] "), *GetNameSafe(InASC), *GetNameSafe(Pawn), *GetNameSafe(InOwnerActor), *GetNameSafe(ExistingAvatar));

	if ((ExistingAvatar != nullptr) && (ExistingAvatar != Pawn))
	{
		UE_LOG(LogA1, Log, TEXT("Existing avatar (authority=%d)"), ExistingAvatar->HasAuthority() ? 1 : 0);
...
void ULyraPawnExtensionComponent::UninitializeAbilitySystem()
{
	...
	if (AbilitySystemComponent->GetAvatarActor() == GetOwner())
	{
		...
		AbilityTypesToIgnore.AddTag(A1GameplayTags::Ability_Behavior_SurvivesDeath);
...
bool ULyraPawnExtensionComponent::CanChangeInitState(UGameFrameworkComponentManager* Manager, FGameplayTag CurrentState, FGameplayTag DesiredState) const
{
	...
	if (CurrentState == A1GameplayTags::InitState_Spawned && DesiredState == A1GameplayTags::InitState_DataAvailable)
	{
	...
	else if (CurrentState == A1GameplayTags::InitState_DataAvailable && DesiredState == A1GameplayTags::InitState_DataInitialized)
	{
		...
		return Manager->HaveAllFeaturesReachedInitState(Pawn, A1GameplayTags::InitState_DataAvailable);
	}
...
{
	if (DesiredState == A1GameplayTags::InitState_DataInitialized)
	{
...
void ULyraPawnExtensionComponent::OnActorInitStateChanged(const FActorInitStateChangedParams& Params)
{
	...
	{
		if (Params.FeatureState == A1GameplayTags::InitState_DataAvailable)
		{
			CheckDefaultInitialization();
		}
	}
