...
void ULyraHealthComponent::InitializeWithAbilitySystem(ULyraAbilitySystemComponent* InASC)
{
	...
	{
		UE_LOG(LogA1, Error, TEXT("LyraHealthComponent: Health component for owner [%s] has already been initialized with an ability system."), *GetNameSafe(Owner));
		return;
	}
	...
	if (!AbilitySystemComponent)
	{
		UE_LOG(LogA1, Error, TEXT("LyraHealthComponent: Cannot initialize health component for owner [%s] with NULL ability system."), *GetNameSafe(Owner));
		return;
	}

	HealthSet = AbilitySystemComponent->GetSet<UA1VitalSet>();
	if (!HealthSet)
	{
		UE_LOG(LogA1, Error, TEXT("LyraHealthComponent: Cannot initialize health component for owner [%s] with NULL health set on the ability system."), *GetNameSafe(Owner));
		return;
	}
	...
	AbilitySystemComponent->SetNumericAttributeBase(UA1VitalSet::GetHealthAttribute(), HealthSet->GetMaxHealth());
...
void ULyraHealthComponent::ClearGameplayTags()
{
	if (AbilitySystemComponent)
	{
		AbilitySystemComponent->SetLooseGameplayTagCount(A1GameplayTags::Status_Death_Dying, 0);
		AbilitySystemComponent->SetLooseGameplayTagCount(A1GameplayTags::Status_Death_Dead, 0);
	}
}
...
void ULyraHealthComponent::OnRep_DeathState(ELyraDeathState OldDeathState)
{
	...
	if (OldDeathState > NewDeathState)
	{
		...
		UE_LOG(LogA1, Warning, TEXT("LyraHealthComponent: Predicted past server death state [%d] -> [%d] for owner [%s]."), (uint8)OldDeathState, (uint8)NewDeathState, *GetNameSafe(GetOwner()));
		return;
	}
	...
	if (OldDeathState == ELyraDeathState::NotDead)
	{
		...
		{
			UE_LOG(LogA1, Error, TEXT("LyraHealthComponent: Invalid death transition [%d] -> [%d] for owner [%s]."), (uint8)OldDeathState, (uint8)NewDeathState, *GetNameSafe(GetOwner()));
		}
	}
...
void ULyraHealthComponent::StartDeath()
{
	...
	if (AbilitySystemComponent)
	{
		AbilitySystemComponent->SetLooseGameplayTagCount(A1GameplayTags::Status_Death_Dying, 1);
	}
...
void ULyraHealthComponent::FinishDeath()
{
	...
	if (AbilitySystemComponent)
	{
		AbilitySystemComponent->SetLooseGameplayTagCount(A1GameplayTags::Status_Death_Dead, 1);
	}
...
void ULyraHealthComponent::DamageSelfDestruct(bool bFellOutOfWorld)
{
	if ((DeathState == ELyraDeathState::NotDead) && AbilitySystemComponent)
	{
		...
		if (!DamageGE)
		{
			UE_LOG(LogA1, Error, TEXT("LyraHealthComponent: DamageSelfDestruct failed for owner [%s]. Unable to find gameplay effect [%s]."), *GetNameSafe(GetOwner()), *ULyraGameData::Get().DamageGameplayEffect_SetByCaller.GetAssetName());
			return;
		}
		...
		if (!Spec)
		{
			UE_LOG(LogA1, Error, TEXT("LyraHealthComponent: DamageSelfDestruct failed for owner [%s]. Unable to make outgoing spec for [%s]."), *GetNameSafe(GetOwner()), *GetNameSafe(DamageGE));
			return;
		}
		...
		Spec->SetSetByCallerMagnitude(A1GameplayTags::SetByCaller_IncomingDamage, DamageAmount);
