...
#include "A1TeamAgentInterface.h"
...
bool ULyraTeamSubsystem::ChangeTeamForActor(AActor* ActorToChange, int32 NewTeamIndex)
{
	...
	else if (IA1TeamAgentInterface* TeamActor = Cast<IA1TeamAgentInterface>(ActorToChange))
	{
...
int32 ULyraTeamSubsystem::FindTeamFromObject(const UObject* TestObject) const
{
	...
	if (const IA1TeamAgentInterface* ObjectWithTeamInterface = Cast<IA1TeamAgentInterface>(TestObject))
	{
	...
	if (const AActor* TestActor = Cast<const AActor>(TestObject))
	{
		...
		if (const IA1TeamAgentInterface* InstigatorWithTeamInterface = Cast<IA1TeamAgentInterface>(TestActor->GetInstigator()))
		{
...
void ULyraTeamSubsystem::AddTeamTagStack(int32 TeamId, FGameplayTag Tag, int32 StackCount)
{
	...
	{
		UE_LOG(LogA1Teams, Error, TEXT("AddTeamTagStack(TeamId: %d, Tag: %s, StackCount: %d) %s"), TeamId, *Tag.ToString(), StackCount, *ErrorMessage);
	};
...
void ULyraTeamSubsystem::RemoveTeamTagStack(int32 TeamId, FGameplayTag Tag, int32 StackCount)
{
	auto FailureHandler = [&](const FString& ErrorMessage)
	{
		UE_LOG(LogA1Teams, Error, TEXT("RemoveTeamTagStack(TeamId: %d, Tag: %s, StackCount: %d) %s"), TeamId, *Tag.ToString(), StackCount, *ErrorMessage);
	};
...
int32 ULyraTeamSubsystem::GetTeamTagStackCount(int32 TeamId, FGameplayTag Tag) const
{
	...
	{
		UE_LOG(LogA1Teams, Verbose, TEXT("GetTeamTagStackCount(TeamId: %d, Tag: %s) failed because it was passed an unknown team id"), TeamId, *Tag.ToString());
		return 0;
	}
}
