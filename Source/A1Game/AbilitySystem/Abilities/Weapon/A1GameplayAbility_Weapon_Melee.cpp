...
#include UE_INLINE_GENERATED_CPP_BY_NAME(A1GameplayAbility_Weapon_Melee)
...
void UA1GameplayAbility_Weapon_Melee::ParseTargetData(const FGameplayAbilityTargetDataHandle& InTargetDataHandle, TArray<int32>& OutCharacterHitIndexes, TArray<int32>& OutBlockHitIndexes)
{
...
void UA1GameplayAbility_Weapon_Melee::ProcessHitResult(FHitResult HitResult, float Damage, bool bBlockingHit, UAnimMontage* BackwardMontage, AA1EquipmentBase* WeaponActor)
{
...
void UA1GameplayAbility_Weapon_Melee::ResetHitActors()
{
	CachedHitActors.Reset();
}

void UA1GameplayAbility_Weapon_Melee::DrawDebugHitPoint(const FHitResult& HitResult)
{
#if UE_EDITOR
	if (GIsEditor)
	{
...
bool UA1GameplayAbility_Weapon_Melee::IsCharacterBlockingHit(ALyraCharacter* TargetCharacter)
{
	UAbilitySystemComponent* TargetASC = UAbilitySystemGlobals::GetAbilitySystemComponentFromActor(TargetCharacter);
	if (TargetASC && TargetASC->HasMatchingGameplayTag(A1GameplayTags::Status_Block))
	{
