...
TSharedPtr<FStreamableHandle> FLyraAssetManagerStartupJob::DoJob() const
{
	...
	UE_LOG(LogA1, Display, TEXT("Startup job \"%s\" starting"), *JobName);
	JobFunc(*this, Handle);
	...
	UE_LOG(LogA1, Display, TEXT("Startup job \"%s\" took %.2f seconds to complete"), *JobName, FPlatformTime::Seconds() - JobStartTime);
