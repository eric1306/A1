...
ULyraAssetManager& ULyraAssetManager::Get()
{
	...
	UE_LOG(LogA1, Fatal, TEXT("Invalid AssetManagerClassName in DefaultEngine.ini.  It must be set to LyraAssetManager!"));
...
{
	UE_LOG(LogA1, Log, TEXT("========== Start Dumping Loaded Assets =========="));

	for (const UObject* LoadedAsset : Get().LoadedAssets)
	{
		UE_LOG(LogA1, Log, TEXT("  %s"), *GetNameSafe(LoadedAsset));
	}

	UE_LOG(LogA1, Log, TEXT("... %d assets in loaded pool"), Get().LoadedAssets.Num());
	UE_LOG(LogA1, Log, TEXT("========== Finish Dumping Loaded Assets =========="));
}
...
const UA1AssetData& ULyraAssetManager::GetAssetData()
{
	return GetOrLoadTypedGameData<UA1AssetData>(AssetDataPath);
}

const UA1UIData& ULyraAssetManager::GetUIData()
{
...
UPrimaryDataAsset* ULyraAssetManager::LoadGameDataOfClass(TSubclassOf<UPrimaryDataAsset> DataClass, const TSoftObjectPtr<UPrimaryDataAsset>& DataClassPath, FPrimaryAssetType PrimaryAssetType)
{
...
#endif
		UE_LOG(LogA1, Log, TEXT("Loading GameData: %s ..."), *DataClassPath.ToString());
	...
	else
	{
		...
		UE_LOG(LogA1, Fatal, TEXT("Failed to load GameData asset at %s. Type %s. This is not recoverable and likely means you do not have the correct data to run %s."), *DataClassPath.ToString(), *PrimaryAssetType.ToString(), FApp::GetProjectName());
	}
...
void ULyraAssetManager::DoAllStartupJobs()
{
	...
	UE_LOG(LogA1, Display, TEXT("All startup jobs took %.2f seconds to complete"), FPlatformTime::Seconds() - AllStartupJobsStartTime);
}
