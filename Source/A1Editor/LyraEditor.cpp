...
DEFINE_LOG_CATEGORY(LogA1Editor);
...
{
	return HasNoPlayWorld() && !GetDefault<UA1DeveloperSettings>()->CommonEditorMaps.IsEmpty();
}
...
static TSharedRef<SWidget> GetCommonMapsDropdown()
{
	...
	for (const FSoftObjectPath& Path : GetDefault<UA1DeveloperSettings>()->CommonEditorMaps)
	{
		if (!Path.IsValid())
		{
			continue;
