...
DEFINE_LOG_CATEGORY_STATIC(LogA1ContentValidation, Log, Log);
...
{
	UE_LOG(LogA1ContentValidation, Display, TEXT("Running ContentValidationCommandlet commandlet..."));
	...
	if (P4FilterString && !P4FilterString->IsEmpty())
	{
		...
		{
			UE_LOG(LogA1ContentValidation, Display, TEXT("ContentValidation returning 1. Failed to get changed files."));
			ReturnVal = 1;
		}
	}
	...
	if (bP4Opened)
	{
		...
		{
			UE_LOG(LogA1ContentValidation, Error, TEXT("P4 workspace was not found when using P4Opened"));
			UE_LOG(LogA1ContentValidation, Display, TEXT("ContentValidation returning 1. Workspace not found."));
			ReturnVal = 1;
		}
	}
...
bool UContentValidationCommandlet::GetAllChangedFiles(IAssetRegistry& AssetRegistry, const FString& P4CmdString, TArray<FString>& OutChangedPackageNames, TArray<FString>& DeletedPackageNames, TArray<FString>& OutChangedCode, TArray<FString>& OutChangedOtherFiles) const
{
	...
	if (LaunchP4(P4CmdString, Results, ReturnCode))
	{
		...
		{
			UE_LOG(LogA1ContentValidation, Error, TEXT("p4 returned non-zero return code %d"), ReturnCode);
		}
	}
...
void UContentValidationCommandlet::GetAllPackagesOfType(const FString& OfTypeString, TArray<FString>& OutPackageNames) const
{
	...
	for (const FString& Type : Types)
	{
		...
		{
			UE_LOG(LogA1ContentValidation, Error, TEXT("Failed to convert short class name \"%s\" to path name. Please use class path names."), *Type);
		}
		else
		{
...
bool UContentValidationCommandlet::LaunchP4(const FString& Args, TArray<FString>& Output, int32& OutReturnCode) const
{
	...
	{
		UE_LOG(LogA1ContentValidation, Error, TEXT("Failed to launch p4."));
	}
...
FString UContentValidationCommandlet::GetLocalPathFromDepotPath(const FString& DepotPathName) const
{
	...
	if (!Workspace.IsEmpty())
	{
		...
		if (LaunchP4(P4WhereCommand, WhereResults, ReturnCode))
		{
			...
			{
				UE_LOG(LogA1ContentValidation, Warning, TEXT("GetAllChangedFiles failed to run p4 'where'. WhereResults[0] = '%s'. Not adding any validation for %s"), WhereResults.Num() > 0 ? *WhereResults[0] : TEXT("Invalid"), *DepotPathName);
			}
		}
	}
